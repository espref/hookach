...
async function initDb() {
  ...
  await run(
    ...
    `CREATE TABLE IF NOT EXISTS orders (
      ...
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      coal_started_at DATETIME,
      coal_changes INTEGER NOT NULL DEFAULT 0,
...
function getCoalInfo(order) {
  const changes = order.coal_changes || 0;
  const requireBowlChange = changes >= 3;
...
// Рандомизировать микс дня (админ)
app.post('/api/admin/mix-of-the-day/random', requireAdmin, async (req, res) => {
  try {
    const mix = await get(db, 'SELECT * FROM mixes ORDER BY RANDOM() LIMIT 1');
    if (!mix) return res.status(404).json({ error: 'Миксы не найдены' });
    await run(db, 'UPDATE mixes SET is_featured = 0');
    await run(db, 'UPDATE mixes SET is_featured = 1 WHERE id = ?', [mix.id]);
    res.json(mix);
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Ошибка сервера' });
...
app.get('/api/orders/:id/status', async (req, res) => {
  try {
    ...
    const coalInfo = getCoalInfo(order);
    res.json({
      status: order.status,
      coalSecondsLeft: coalInfo.coalSecondsLeft,
      coalChanges: coalInfo.coalChanges,
      requireBowlChange: coalInfo.requireBowlChange
    });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Ошибка сервера' });
...
app.post('/api/admin/orders/:id/status', requireAdmin, async (req, res) => {
  try {
    ...
    if (status === 'heating') {
      await run(
        db,
        'UPDATE orders SET status = ?, coal_started_at = CURRENT_TIMESTAMP, is_active = 1 WHERE id = ?',
        [status, id]
      );
    } else {
      await run(db, 'UPDATE orders SET status = ?, is_active = 1 WHERE id = ?', [
        status,
        id
      ]);
    }
...
app.post('/api/admin/orders/:id/done', requireAdmin, async (req, res) => {
  try {
    const id = Number(req.params.id);
    await run(db, 'UPDATE orders SET status = ?, is_active = 0 WHERE id = ?', [
      'done',
      id
    ]);
    res.json({ ok: true });
...
// Смена углей
app.post('/api/admin/orders/:id/coal-change', requireAdmin, async (req, res) => {
  try {
    const id = Number(req.params.id);
    const order = await get(db, 'SELECT * FROM orders WHERE id = ?', [id]);
    if (!order) return res.status(404).json({ error: 'Заказ не найден' });

    const currentChanges = order.coal_changes || 0;
    if (currentChanges >= 3) {
      const info = getCoalInfo(order);
      return res.json({ ...order, ...info, requireBowlChange: true });
    }

    await run(
      db,
      'UPDATE orders SET coal_changes = coal_changes + 1, coal_started_at = CURRENT_TIMESTAMP WHERE id = ?',
      [id]
    );
    const updated = await get(db, 'SELECT * FROM orders WHERE id = ?', [id]);
    const info = getCoalInfo(updated);
